...
def test_chat(page: Page, live_server_url: str):
    ...
    expect(page.get_by_role("button",  id="clear-chat")).to_be_disabled()
    expect(page.get_by_role("button", name="Developer settings")).to_be_enabled()

    # Ask a question and wait for the message to appear
    # page.get_by_placeholder("Posez votre question. Essayez d'être le plus précis possible dans votre demande.").click()
    # page.get_by_placeholder("Posez votre question. Essayez d'être le plus précis possible dans votre demande.").fill(
    #     "Qu'est ce qu'un transitaire en douane?"
    # )
    # page.get_by_role("button", name="Ask question button").click()

    # expect(page.get_by_text("Qu'est ce qu'un transitaire en douane?")).to_be_visible()
    # expect(page.get_by_text("The capital of France is Paris.")).to_be_visible()
    expect(page.get_by_role("button", id="clear-chat")).to_be_enabled()
